...
import { getUpcomingPaymentDueDate, isPaymentDoneForCurrentStatement } from "@/utils/date-helpers";
...
const GroupedPaymentDueDatesCard: React.FC<GroupedPaymentDueDatesCardProps> = ({ cards }) => {
    ...
    return (
      <Card className={cardBaseClasses}>
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          <CardTitle className="text-lg font-bold text-blue-800">¡Vas bien!</CardTitle>
        </CardHeader>
        <CardContent className="flex flex-col items-center pr-4 md:block md:pr-48">
          ...
          <div className="text-lg font-bold text-center md:text-left">
            No hay pagos de tarjetas<br />pendientes para los<br />próximos 30 días.
          </div>
          <p className="text-xs text-blue-700 mt-1 text-center md:text-left flex items-center gap-1 justify-center md:justify-start">
            <CheckCircle2 className="h-3 w-3" /> ¡Todo está al día!
          </p>
        </CardContent>
      </Card>
    );
  ...
  return (
    <Card className={cardBaseClasses}>
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
        <CardTitle className="text-lg font-bold text-blue-800">Tu calendario de pagos</CardTitle>
      </CardHeader>
      <CardContent className="flex flex-col items-center pr-4 md:block md:pr-48">
        <img src="https://nyzquoiwwywbqbhdowau.supabase.co/storage/v1/object/public/Media/Cochinito%20Calendario.png" alt="Calendario" className="h-[180px] w-[180px] mb-4 md:absolute md:top-[-10px] md:right-4 md:z-10" />
        {upcomingPayments.map((payment, index) => {
          ...
          } else if (payment.daysRemaining < 0) {
            message = `¡El pago de tu tarjeta ${payment.cardName} está vencido!`;
          ...
          return (
            <div key={index} className={cn("mb-2 last:mb-0 text-center md:text-left", textColorClass)}>
              ...
              <p className="text-xs">Fecha límite: <span className="font-semibold">{formattedDueDate}</span></p>
            </div>
          );
        })}
      </CardContent>
