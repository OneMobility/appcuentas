...
const LucideIconComponents: { [key: string]: React.LazyExoticComponent<ComponentType<any>> } = {};
  ...
  if (!LucideIconComponents[iconName]) {
    LucideIconComponents[iconName] = lazy(() =>
      import('lucide-react').then(module => {
        const IconComponent = module[iconName as keyof typeof module] as ComponentType<any>;
        if (IconComponent) {
          ...
          return { default: module.Tag as ComponentType<any> };
        }
      }).catch(error => {
        console.error(`Error al cargar el icono "${iconName}":`, error);
        // Fallback a un icono por defecto en caso de error de carga
        return import('lucide-react').then(module => ({ default: module.Tag as ComponentType<any> }));
      })
    );
  }

  const Icon = LucideIconComponents[iconName] as any;
