...
import { DollarSign, Trash2, Edit, ArrowLeft, FileDown, Search, Filter, FileText, ChevronLeft, ChevronRight } from "lucide-react";
...
const CreditorDetailsPage: React.FC = () => {
  ...
  const handleTransactionSubmit = async (e: React.FormEvent) => {
    ...
    let finalAmount = currency === "USD" ? baseAmount * usdToMxnRate : baseAmount;
  ...
  return (
    <div className="flex flex-col gap-6 p-2 sm:p-4 md:p-6 pb-24 max-w-5xl mx-auto">
      <div className="flex items-center gap-4">
        <Button variant="ghost" size="icon" className="rounded-full" onClick={() => navigate('/creditors')}><ArrowLeft className="h-5 w-5" /></Button>
        <div>
          <h1 className="text-2xl font-black tracking-tight">{creditor.name}</h1>
          <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">Detalle de Deuda</p>
        </div>
      </div>
      ...
      <Card className="rounded-3xl border-none shadow-sm bg-white overflow-hidden">
        <CardHeader className="flex flex-col sm:flex-row sm:items-center justify-between p-4 bg-slate-50 gap-2">
          ...
          <Button size="sm" className="h-9 gap-1 rounded-xl font-bold bg-rose-600 hover:bg-rose-700 text-white" onClick={() => { setEditingTransaction(null); setIsTransactionDialogOpen(true); }}>
                        ...
                        <AlertDialog>
                          ...
                          <AlertDialogContent className="rounded-2xl w-[90vw] max-w-md">
                            ...
                            <AlertDialogFooter>
                              ...
                              <AlertDialogAction onClick={() => handleDeleteTransaction(tx)} className="rounded-xl bg-rose-600">Eliminar</AlertDialogAction>
                            </AlertDialogFooter>
                          </AlertDialogContent>
                        </AlertDialog>
                      </div>
